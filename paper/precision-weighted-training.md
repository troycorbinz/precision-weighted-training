...
**Layer 0 exclusion.** Layer 0 is structurally unique — it bridges token embeddings and transformer representations, and its forward-pass divergence is dominated by this embedding-to-representation transformation rather than ongoing refinement. Early in our Phase 3 experiments we observed that layer 0 had divergence 10–25× higher than the mean across other layers, which caused the normalization factor $\bar{d}$ to be pulled upward and systematically attenuated every other layer's gradient. Excluding layer 0 from the normalization (it still trains normally with scale=1.0) fixed this. This is the only hand-tuned exception; all other layers participate.
...
**Hardware.** Both runs on a single NVIDIA RTX 5090 (32GB VRAM), ~9,600 tokens/sec. Approximately 7.5 days per run. (Phase 1 and Phase 2 experiments were conducted on an RTX 3090.)
...
| Step | L0 | L3 | L7 | L10 | L15 | L19 | L0/rest |
|---|---|---|---|---|---|---|---|
...
**Layer 19** (the final block) grew from 0.123 to 0.597 — a **387% increase** — and was *still growing* at step 30K with no sign of plateau. Late-stage representation refinement is active and increasing.

**Middle layers (L7–L18)** stayed in a low divergence band (~0.09–0.32) with no sustained growth. These are the "refinement" layers, making consistent small adjustments. L4–L6 sit in a transitional band (final divergence 0.35–0.42) between the actively-growing early zone and the stable middle.
...
The central finding of this paper — that **aggregate val loss is not sufficient to evaluate gain-function-style interventions** — is supported at both scales. Phase 1 established this at 50M parameters: A2 (focal) achieved better val_loss than A0 at some checkpoints while producing degenerate output. Phase 3 establishes it at 1.2B parameters and 24× larger training tokens: baseline and gain have statistically indistinguishable smoothed val loss (difference 0.004, noise ±0.2), yet the gain model is preferred 63.4% of the time in blind A/B comparison (p = 1.98 × 10⁻⁵).
