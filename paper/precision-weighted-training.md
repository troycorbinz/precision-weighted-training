...
**Two language models trained on identical data in identical order can converge to the same smoothed validation loss yet differ noticeably in the quality of what they generate, and the metric most LLM training literature reports won't see it.** In this paper *quality* is defined operationally as blind A/B preference rate by a 42-judge panel of humans and foundation-model judges. In a controlled 1.2B-parameter comparison at 3.9B tokens (16.4% of Chinchilla-optimal), a Predictive-Coding-inspired reshaping of the per-token learning signal achieves smoothed val loss within 0.004 of baseline yet is preferred in **59.9% of 784 decisive blind A/B comparisons** across 1,181 judgments by that panel (p = 2.80 × 10⁻⁸, two-sided binomial; direction survives every sensitivity filter we apply).
...
This paper develops and empirically characterizes two training-time interventions, per-token precision-weighted gain and per-layer divergence-scaled gradients, that together give the model's own prediction error and representation-change signal the job of shaping the learning signal. Throughout, *quality* refers to the operational measure used in our evaluation: the rate at which one model's outputs are preferred over another's in blind A/B comparisons by a 42-judge panel of humans and foundation-model judges (full setup in Section 7.1). We make three contributions:
...
**Layer 0** remained the structural outlier (1.2–1.8 across training, sitting at 1.3–1.5 for most of the run after an early-step peak at 1.77). As the embedding-to-representation bridge, its ratio fell from ~12× the sample mean at step 1K to ~4× by step 10K and then stabilized. Its consistent saturation at $m_\max = 3.0$ is the mechanism signal described in Section 4.2, the driver of mid-layer scale compression.
...
- Helm, F., Daheim, N., & Gurevych, I. (2025). Token Weighting for Long-Range Language Modeling. *Findings of NAACL 2025*. *arXiv preprint arXiv:2503.09202*.
